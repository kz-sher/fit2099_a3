...
public class Take extends SWAffordance {
	...
	public void act(SWActor a) {
		if (target instanceof SWEntityInterface) {
			...
			//remove the take affordance and add the leave affordance
			target.removeAffordance(this);
			target.addAffordance(new Leave(theItem,this.messageRenderer));
			
			//if target is grenade, add the throw affordance
			if(target instanceof Grenade) {
				target.addAffordance(new Throw(theItem, this.messageRenderer));
			}
