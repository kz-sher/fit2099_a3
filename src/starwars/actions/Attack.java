		...
		if (targetIsActor && (a.getTeam() == targetActor.getTeam())) { //don't attack SWActors in the same team
			...
			if (itemCarried != null) {//if the actor is carrying an item 
				if (itemCarried.hasCapability(Capability.WEAPON)) {
					...
					//if the actor is luke and he is holding the lightsaber 
					//but he does not have enough force level to wield the lightsaber 
					if(a.getSymbol() == "@" && itemCarried.getSymbol() == "†" && 
						!(a.hasCapability(Capability.FORCE) && a.getForceLevel() > 0)){
						System.out.println("\t" + a.getShortDescription() + " says: I can't wield the light sabre!\n" 
							  + "\t\t >>>My force level: " + a.getForceLevel() + "\n"
							  + "\t\t >>>(Force level has to be more than one in order to wield the lightsabre)");
						return;
					}
					
					//if the actor takes other weapon
			...
			if (target.getHitpoints() <= 0) { // can't use isDead(), as we don't know that the target is an actor
				
				if(targetIsActor) { // if target is an actor, then change his description
